...
int BinarySearch(const int arr[], int low, int high, int key) {
  while (low <= high) { // Mientras el índice bajo sea menor o igual al alto, es decir, mientras haya elementos en el arreglo
    int mid = low + (high - low) / 2;  // Calcula el índice medio o central
    if (arr[mid] == key) { // Si el valor en el índice medio es igual al valor buscado
      return mid;  // Retorna el índice si encuentra el valor
    }
    if (arr[mid] < key) { // Si el valor en el índice medio es menor al valor buscado
